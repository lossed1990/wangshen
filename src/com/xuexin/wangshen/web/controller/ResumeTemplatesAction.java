...
public class ResumeTemplatesAction {
	...
	public String resumeTemplateNew(Model model, HttpServletRequest request, RedirectAttributes attr,
		...
		// 简单参数检查
		if (strTemplName == null || strTemplName.length() == 0 || strCoverID == null || strCoverID.length() == 0) {

			// 作为运行时错误
			model.addAttribute("error", WebContextResouceBundleReader
					.makeRuntimeErrorInfoByCode(Integer.toHexString(ErrorDefines.E_SVR_RUNTIME_BADINPUT), request));
	...
	public CommonResultDTO resumeTemplatePartSave(@RequestBody ResumeTemplatePartDTO modpart) {
		...
		int nRet = service_tmpl.updateTemplateJSON(modpart);
